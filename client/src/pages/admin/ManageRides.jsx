...
import { FaRoute, FaArrowLeft, FaMapMarkerAlt, FaCalendarAlt } from 'react-icons/fa';
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h2 className="text-xl md:text-3xl font-black text-slate-100 uppercase tracking-tight flex items-center space-x-2">
            ...
            <span>Booking Ledger Logs</span>
          </h2>
          <p className="text-xs md:text-sm text-slate-400 font-medium">Verify system-wide coordinator transactions and locations histories</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass p-12 text-center rounded-2xl border border-white/5 shadow-xl">
          <p className="text-slate-400 text-sm font-semibold">No rides have been requested on this platform yet.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {rides.map((ride) => (
            <div key={ride.id} className="glass p-5 rounded-2xl border border-white/5 shadow-xl space-y-4">
              {/* Header */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-medium">
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="text-[10px] text-slate-500 font-bold block mb-0.5">DESTINATION</span>
